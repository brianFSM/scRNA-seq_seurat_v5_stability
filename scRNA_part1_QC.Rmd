...
# Shared output root. Part 1's object, QC tables, and raw QC figures are
# cutoff-independent, so they live here rather than inside a run directory.
# Parts 2a/2b/3 write to <rds-file-path>/<run_name> instead - see run_dir().
r.obj.loc <- config.args$data$`rds-file-path`

# part1_suffix keeps genuine part-1 variants apart (e.g. SoupX on vs off).
# It is NOT the knob for QC-cutoff variants; those get their own run_name.
part1.suffix <- config.args$analysis$part1$part1_suffix

unfiltered.seurat.obj.path <- suffix_path(
  r.obj.loc, config.args$analysis$part1$part1_rds_save_filename, part1.suffix)
QC.tables.path <- suffix_path(
  r.obj.loc, config.args$analysis$part1$part1_report_tables_filename, part1.suffix)
